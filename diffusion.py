...
class Diffusion(nn.Module):
    def __init__(self, embed_size: int, context_size: int, head_size: int, head_count: int, n_unet_reductions: int = 4, num_groups: int = 32, **unet_kwargs):
        super().__init__()
    ...
    def forward(self, latent_image_embeddings: torch.Tensor, text_embeddings: torch.Tensor, time: torch.Tensor) -> torch.Tensor:
