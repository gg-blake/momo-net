...
class SelfAttentionHead(nn.Module):
    ...
    def __init__(
        ...
        self.query_weights = nn.Linear(embed_size, head_size, bias=proj_bias)
        self.key_weights = nn.Linear(embed_size, head_size, bias=proj_bias)
        self.value_weights = nn.Linear(embed_size, head_size, bias=proj_bias)
...
class SelfAttention(nn.Module):
    def __init__(
        ...
        self.heads = nn.ModuleList(
            [
                SelfAttentionHead(embed_size, head_size // head_count, dropout)
...
class CrossAttentionHead(nn.Module):
    ...
    def __init__(
        ...
        self.q_size = q_size
        self.query_weights = nn.Linear(q_size, head_size, bias=proj_bias)
        self.key_weights = nn.Linear(kv_size, head_size, bias=proj_bias)
        self.value_weights = nn.Linear(kv_size, head_size, bias=proj_bias)
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, queries: torch.Tensor, context: torch.Tensor) -> torch.Tensor:
        ...
        wei = Q @ K.transpose(-2, -1) * self.q_size**-0.5  # [B, T_q, T_kv]
        wei = torch.softmax(wei, dim=-1)
...
class CrossAttention(nn.Module):
    ...
    def __init__(
        self, q_size: int, kv_size: int, head_size: int, head_count: int, dropout: float
    ):
        super().__init__()
        self.heads = nn.ModuleList(
            [
        ...
        self.proj = nn.Linear(q_size, q_size)
        self.dropout = nn.Dropout(dropout)
